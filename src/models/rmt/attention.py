...
class SHA(AbstractSowableModule):
    ...
    def make_attn_bias(self, mask: hax.NamedArray):
        m = hax.geomspace(self.Rank, start=2**(-8/self.Rank.size), stop=2**(-8))
        bias = -(hax.cumsum(mask[{'KVPos': slice(None, None, -1)}], axis='KVPos')[{'KVPos': slice(None, None, -1)}] - 1)
        bias = bias.broadcast_axis(self.Rank) * m
    ...
    def forward(
        ...
        query, key, value = hax.tree_util.tree_map(lambda x: x.rearrange((..., 'Rank', 'Pos', 'ResVal')), (query, key, value))
        key, value = hax.tree_util.tree_map(lambda x: x.rename({'Pos': 'KVPos'}), (key, value))
        attn_logits = hax.dot(query, key, axis='ResVal', precision=jax.lax.Precision.HIGHEST)
    ...
    def init(
        ...
        return cls(
            mup_scaling=config.attn_multiplier * (math.sqrt(config.ResVal.base_fan_dim)/config.ResVal.fan_dim),
            Rank=config.Rank.axis,
            policy=jmp.Policy(
